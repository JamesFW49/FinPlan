...
function PendingCard(props) {
  ...
  var onConfirm = props.onConfirm; // opens the month-end confirmation modal
  ...
  return h("div", {
    ...
    h("span", { style: { color: C.textLo, fontSize: 11 } },
      pending ? "This month\u2019s " + label.toLowerCase() + " not yet confirmed" : "Confirmed for " + monthKeyToLabel(currentMonthKey())
    ),
    pending && onConfirm ? h("button", {
      onClick: onConfirm,
      style: {
...
function Dashboard(props) {
  const data = props.data, setData = props.setData, onNavigate = props.onNavigate, onConfirm = props.onConfirm;
  ...
  return h("div", { style: { display: "flex", flexDirection: "column", gap: 20 } },
    ...
    h("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 12 } },
      h(PendingCard, { label: "Investments", amount: mInvest, pending: pending, onConfirm: pending ? onConfirm : undefined }),
      h(PendingCard, { label: "Debt Payments", amount: mDebtPay, pending: pending, onConfirm: pending ? onConfirm : undefined })
    ),
...
// ─── PAGE: DEBT ───────────────────────────────────────────────────────────────
function Debt(props) {
  const data = props.data, setData = props.setData, onConfirm = props.onConfirm;
  ...
  return h("div", { style: { display: "flex", flexDirection: "column", gap: 20 } },
    ...
    h(PendingCard, { label: "Debt Payments", amount: totalPay, pending: isPendingThisMonth(data), onConfirm: isPendingThisMonth(data) ? onConfirm : undefined }),
    h(Panel, null,
...
function Investments(props) {
  ...
  return h("div", { style: { display: "flex", flexDirection: "column", gap: 20 } },
    ...
    h(PendingCard, { label: "Investments", amount: totalContrib, pending: isPendingThisMonth(data), onConfirm: isPendingThisMonth(data) ? onConfirm : undefined }),
    h(Panel, null,
...
function LockScreen(props) {
  ...
  return h("div", {
    ...
    h("img", { src: "icon-192.png", style: { width: 72, height: 72, borderRadius: 18 } }),
...
function App() {
  ...
  // Manual "Confirm this month" trigger — sets pendingClose to the current
  ...
  if (page === "dashboard") pageContent = h(Dashboard, { data: data, setData: setData, onNavigate: setPage, onConfirm: triggerConfirm });
  else if (page === "transactions") pageContent = h(Transactions, { data: data, setData: setData });
  else if (page === "debt") pageContent = h(Debt, { data: data, setData: setData, onConfirm: triggerConfirm });
  else if (page === "investments") pageContent = h(Investments, { data: data, setData: setData, onConfirm: triggerConfirm });
